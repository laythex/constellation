...
namespace Constants {
	...
	constexpr double ATMO_HEIGHT							= 1e5;
	...
	const sf::Vector2i WINDOW_POSITION						= {static_cast<int>(SCREEN_WIDTH - WINDOW_WIDTH) / 2,
															   static_cast<int>(SCREEN_HEIGHT - WINDOW_HEIGHT) / 2};
	...
	constexpr unsigned ORBIT_DETAILING						= 1e2;

	constexpr double SMA_CHANGE_RATE						= 1e7;
	...
	const sf::Vector2f FPS_DISPLAY_POSITION					= sf::Vector2f(-WINDOW_WIDTH / 2 + TEXT_OFFSET, -WINDOW_HEIGHT / 2 + TEXT_OFFSET);
	const sf::Vector2f BALANCE_DISPLAY_POSITION				= sf::Vector2f(WINDOW_WIDTH / 2 - TEXT_OFFSET * 2, -WINDOW_HEIGHT / 2 + TEXT_OFFSET);
	const sf::Vector2f LAUNCH_PRICE_DISPLAY_POSITION		= sf::Vector2f(0, -WINDOW_HEIGHT / 2 + TEXT_OFFSET * 3);
	const sf::Vector2f DATE_DISPLAY_POSITION				= sf::Vector2f(0, WINDOW_HEIGHT / 2 - TEXT_OFFSET * 5);

	const sf::Vector2f TUTORIAL_PLATE_SIZE					= sf::Vector2f(490, 340);
	const sf::Vector2f TUTORIAL_PLATE_POSITION				= sf::Vector2f(-WINDOW_WIDTH / 2 + 50,
																		   static_cast<float>(WINDOW_HEIGHT / 2 - TUTORIAL_PLATE_SIZE.y - 50));
